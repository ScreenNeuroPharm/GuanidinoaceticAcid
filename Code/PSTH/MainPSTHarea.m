...
mcmea_electrode = MEA120_lookuptable;
...
for i = first:num_dir               % FOR cycle over all the directories
    ...
    psth_area= zeros (120,1);         % A vector containing the values of the PSTH area is allocated (87x2)
    ...
    for k=3:num_files              % FOR cycle over all the PSTH files
        ...
        el= split(filename,'.');
        el = split(el{1},'_');
        el = el{end};           % current electrode [char]
        ch_index = double(mcmea_electrode(strcmp(mcmea_electrode(:,1),el),2));
        %ch_index= find(mcmea_electrodes==str2num(electrode));
        
        if (sum(psthcnt)>= minarea)
            psth_area(ch_index)=sum(psthcnt); % Compute the PSTH integral
        else
            psth_area(ch_index)= 0;
        end

    end

    %psth_area(:,1)=mcmea_electrodes(:,1);    % First column = electrode names  
